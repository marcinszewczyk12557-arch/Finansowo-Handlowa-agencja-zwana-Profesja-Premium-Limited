...
type DisplayOffer = PublicOffer & { subcategory: string };
...
export default function HierarchicalCatalog() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const response = await fetch('/api/catalog/qualified-offers', { cache: 'no-store' });
        const data = await response.json();
        if (!response.ok || !data.ok || !Array.isArray(data.offers)) throw new Error('Nie udało się pobrać katalogu.');
        const safeOffers = data.offers.map((offer: PublicOffer) => ({
          id: String(offer.id || ''),
          ...
          supplierYears: Math.max(0, Number(offer.supplierYears) || 0),
          subcategory: subcategoryFor(String(offer.category || ''), String(offer.title || '')),
        })).filter((offer: DisplayOffer) => offer.id && offer.category && offer.title);
        if (!active) return;
        setOffers(safeOffers);
        setSelectedCategory((current) => current || safeOffers[0]?.category || '');
      } catch (error) {
        if (active) setLoadError(error instanceof Error ? error.message : 'Błąd katalogu.');
      } finally {
  ...
  const effectiveSubcategory = selectedSubcategory && subcategories.includes(selectedSubcategory)
    ? selectedSubcategory
    : (subcategories[0] ?? '');
  ...
  const visible = useMemo(() => {
    ...
    return offers.filter((offer) => offer.category === selectedCategory
      && (!effectiveSubcategory || offer.subcategory === effectiveSubcategory)
      && (!q || `${offer.title} ${offer.use} ${offer.purpose} ${offer.function} ${offer.subcategory}`.toLowerCase().includes(q)));
  ...
  return <>
    <section className='section catalog-taxonomy-summary'>
      ...
      <p className='catalog-count'>Każda profesjonalna oferta przechodzi wewnętrzną kwalifikację źródła i dostawcy. Dane źródłowe, profile dostawców i odnośniki do producentów pozostają wyłącznie w zapleczu PROFESJA i nie są przesyłane do publicznej aplikacji klienta. Cena, gwarancja, dokumentacja, multimedia i parametry są ponownie potwierdzane przed finalnym RFQ.</p>
    </section>
      ...
      <div className='taxonomy-layout'>
        ...
        <div className='taxonomy-products'>
          <section className='taxonomy-leaf'>
            ...
            {loading ? <div className='catalog-empty'>Ładowanie zweryfikowanego asortymentu…</div> : null}
            {loadError ? <div className='catalog-empty' role='alert'>{loadError}</div> : null}
            {!loading && !loadError && offers.length === 0 ? <div className='catalog-empty'>Brak ofert z kompletnym zestawem dowodów. Kandydaci pozostają w kwalifikacji wewnętrznej i nie są publikowani do czasu potwierdzenia wszystkich warunków.</div> : null}
            ...
            <div className='taxonomy-product-grid'>
              {visible.map((offer, index) => <article className='taxonomy-product-card professional-offer-card' key={offer.id}>
                ...
                <p className='eyebrow'>ZWERYFIKOWANY DOSTAWCA • {offer.supplierYears}+ LAT • OCHRONA TRANSAKCJI WERYFIKOWANA DLA KONKRETNEJ OFERTY</p>
                <h3>{offer.title}</h3>
                ...
                <div className='source-dossier'>
                  ...
                  <p>Źródło zakupu, dane producenta, profile dostawców, dokumenty weryfikacyjne oraz zewnętrzne adresy pozostają informacją wewnętrzną PROFESJA. Publiczny interfejs otrzymuje wyłącznie bezpieczny zestaw danych handlowych bez linków źródłowych i nazw dostawców.</p>
                </div>
