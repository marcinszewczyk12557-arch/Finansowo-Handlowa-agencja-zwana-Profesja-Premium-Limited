...
const STORE01_MIN_ORDER_PLN = 110000;
...
export default function StoreProductBrowser({ products, category }: Props) {
  ...
  return (
    <>
      <div style={{ background: '#fff', border: '1px solid #dde3e6', borderRadius: 18, padding: 18, margin: '20px 0' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(240px,1fr))', gap: 12 }}>
          <label style={{ display: 'grid', gap: 7, fontWeight: 700 }}>
            ...
            <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="np. 01-025, 5G, rugged, USB-C" aria-label="Szukaj produktu po numerze, nazwie lub zastosowaniu" style={{ minHeight: 46, border: '1px solid #cfd8dc', borderRadius: 10, padding: '0 12px', font: 'inherit' }} />
          </label>
          <label style={{ display: 'grid', gap: 7, fontWeight: 700 }}>
            Dział / Department
            <select value={department} onChange={(event) => setDepartment(event.target.value)} aria-label="Filtruj produkty według działu" style={{ minHeight: 46, border: '1px solid #cfd8dc', borderRadius: 10, padding: '0 12px', font: 'inherit', background: '#fff' }}>
              <option value="ALL">Wszystkie działy ({products.length})</option>
              {departments.map((item) => <option key={item} value={item}>{item}</option>)}
            </select>
          </label>
        </div>
        <div style={{ marginTop: 12, color: '#52636b' }}>Wyniki: <strong>{filtered.length}</strong> / {products.length}</div>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(280px,1fr))', gap: 14 }}>
        {filtered.map((product) => {
          const sourceStore = product.code.split('-')[0] || '';
          const minOrderQuery = sourceStore === '01' ? `&budget=${STORE01_MIN_ORDER_PLN}` : '';
          return (
            <article key={product.code} className="professional-offer-card" style={{ background: '#fff', border: '1px solid #dde3e6', borderRadius: 18, padding: 22 }}>
              ...
              <div style={{ borderTop: '1px solid #e3e8ea', paddingTop: 12, marginTop: 14, color: '#52636b', fontSize: '.9rem', lineHeight: 1.55 }}>
                ...
                {sourceStore === '01' ? <><br /><strong>Minimalna wartość pojedynczego zamówienia sklepu 01: 110 000 zł.</strong></> : null}
              </div>
              <Link href={`/offers/new?product=${encodeURIComponent(product.title)}&category=${encodeURIComponent(category)}&productNumber=${encodeURIComponent(product.code)}&store=${encodeURIComponent(sourceStore)}${minOrderQuery}`} style={{ fontWeight: 700, display: 'inline-block', marginTop: 14 }}>
      ...
      {filtered.length === 0 ? <div style={{ background: '#fff', border: '1px solid #dde3e6', borderRadius: 16, padding: 24 }}>Brak pozycji spełniających ten filtr. Zmień dział lub wyszukiwane hasło.</div> : null}
    </>
  );
}
