...
type SubmissionResult = { number: string; product: string; status: string; createdAt: string };
...
export default function OfferForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const payload = { ...Object.fromEntries(form.entries()), sourceStore: store, financingRequested: form.get('financingRequested') === 'on' };
    ...
    try {
      ...
      setProduct(''); setBudget(''); setStore(''); setFinancing(false);
    } catch (err) {
  ...
  return (
    <form onSubmit={submit} className="premium-form">
      ...
      <label style={{ display: 'block', marginTop: 12 }}><input name="financingRequested" type="checkbox" checked={financing} onChange={(event) => setFinancing(event.target.checked)} /> Chcę, aby zapytanie obejmowało również organizację finansowania B2B</label>
      {financing ? <label>Orientacyjna kwota finansowania<input name="financingAmount" placeholder="np. 110 000 PLN" /></label> : null}
      ...
      <section className="admin-note" style={{ marginTop: 20 }}><h3>Procedura zarządczo-wykonawcza</h3><p>Po utworzeniu sprawy system tworzy pakiet formalności powiązany z numerem `PPL-...`. Dane wynikające z negocjacji mogą być uzupełniane wraz z przebiegiem transakcji.</p><p>Automatyczne uzupełnienie danych nie oznacza zaakceptowania oświadczeń ani podpisania dokumentów.</p></section>
      <section className="admin-note" style={{ marginTop: 20 }}><h3>Przed wysłaniem i dalszą finalizacją</h3><p>Wiążące warunki transakcji wynikają z zaakceptowanej oferty, potwierdzenia zamówienia lub umowy.</p><p>Sposób i koszt dostawy są ustalane dla konkretnej transakcji.</p><p>Zapoznaj się z <Link href="/company">danymi przedsiębiorcy i zasadami procesu</Link>, <Link href="/terms">Regulaminem</Link> oraz <Link href="/privacy">Polityką prywatności</Link>.</p></section>
      <label style={{ display: 'block', marginTop: 14 }}><input name="transactionInformationAcknowledged" type="checkbox" required /> Potwierdzam zapoznanie się z informacjami o przedsiębiorcy i procesie.</label>
      <label style={{ display: 'block', marginTop: 10 }}><input name="privacyAcknowledged" type="checkbox" required /> Potwierdzam zapoznanie się z Regulaminem i Polityką prywatności.</label>
      <button type="submit" disabled={sending}>{sending ? 'ZAPISYWANIE ZAPYTANIA...' : 'WYŚLIJ ZAPYTANIE B2B'}</button>
      {result ? <div className="form-status" role="status"><strong>Zapytanie zostało przyjęte.</strong><br />Numer sprawy: <b>{result.number}</b><br />Status: {result.status}.<div className="cta-row" style={{ marginTop: 16 }}><Link href="/dashboard"><button type="button">PRZEJDŹ DO PANELU KLIENTA</button></Link><Link href="/catalog"><button type="button" className="cta-secondary">WRÓĆ DO KATALOGU</button></Link></div></div> : null}
