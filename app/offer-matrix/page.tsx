...
import { CATALOG_ITEMS_PER_STORE, CATALOG_TOTAL_ITEMS, getCatalogPage, scalableCatalogCategories } from '../../data/scalableCatalog';
...
export default async function OfferMatrixPage({ searchParams }: { searchParams?: SearchParams }) {
  ...
  return <><Header/><main style={{background:'#eef1f3',color:'#172126',minHeight:'100vh'}}>
    <section style={{background:'linear-gradient(135deg,#10191e,#26343b)',color:'#fff',padding:'56px 24px'}}><div style={{maxWidth:1280,margin:'0 auto'}}>
      ...
      <p style={{maxWidth:980,lineHeight:1.7,color:'#d5dde0'}}>Each specialist store has its own thematic identity displayed on every grey product card assigned to that store. One grey card represents one individual catalogue position. Exact supplier logos and third-party trademarks are not implied.</p>
      <p style={{maxWidth:980,lineHeight:1.7,color:'#bcc8cd'}}><strong>PL:</strong> Każdy z 50 sklepów posiada własną identyfikację tematyczną widoczną na każdej przypisanej mu szarej karcie. Jedna szara karta oznacza jedną indywidualną pozycję katalogową.</p>
    ...
    <section style={{maxWidth:1280,margin:'0 auto',padding:'18px 20px 64px'}}><div style={{display:'flex',justifyContent:'space-between',gap:16,flexWrap:'wrap',marginBottom:18}}><div><p style={{fontWeight:900,color:'#8a7440',marginBottom:4}}>CATALOGUE / KATALOG</p><h2 style={{margin:0}}>{category||'All specialist stores / Wszystkie sklepy'}</h2><p>{result.totalItems.toLocaleString('pl-PL')} pozycji • strona {result.page}/{result.totalPages}</p></div></div>
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(270px,1fr))',gap:14}}>{result.items.map(item=>{const logo=storeLogo(item.storeNo,item.category);return <article key={item.id} style={{background:'#d9dde0',border:'1px solid #c3c9cd',borderRadius:16,padding:18,boxShadow:'0 10px 28px rgba(18,30,36,.08)'}}>
        ...
        <div style={{fontSize:13,lineHeight:1.55}}><p><strong>Role / Rola:</strong> {item.role}</p><p><strong>Form:</strong> {item.formFactor}</p><p><strong>Door-to-door:</strong> {item.delivery}</p><p><strong>Warranty:</strong> {item.warranty}</p><p><strong>Service:</strong> {item.service}</p><p><strong>Consumables:</strong> {item.consumables}</p></div>
        <div style={{display:'flex',gap:10,flexWrap:'wrap',marginTop:14}}><a href={item.sourceUrl} target="_blank" rel="noreferrer" style={{fontWeight:900,color:'#172126'}}>Alibaba sourcing →</a><Link href={`/offers/new?product=${encodeURIComponent(item.titlePl)}&category=${encodeURIComponent(item.category)}`} style={{fontWeight:900,color:'#7a672f'}}>RFQ →</Link></div>
      </article>})}</div>
