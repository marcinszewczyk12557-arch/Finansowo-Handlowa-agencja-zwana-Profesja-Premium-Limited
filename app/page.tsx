...
const privateLabelPrograms = [
  { name: 'Private Label — Smartfony i urządzenia mobilne', moq: 'MOQ potwierdzane indywidualnie dla wybranego projektu', strengths: 'logo klienta • ekran startowy • opakowanie • etykieta • konfiguracja pamięci i sieci' },
  { name: 'ODM — Elektronika użytkowa', moq: 'MOQ zależne od konfiguracji, funkcjonalności i zakresu personalizacji', strengths: 'projekt produktu • branding • firmware • aplikacje • akcesoria • dokumentacja' },
  { name: 'OEM — Serie produktowe B2B', moq: 'wielkość partii ustalana dla konkretnego modelu i specyfikacji', strengths: 'hardware • software • warianty pamięci • kolory • oznakowanie • instrukcje' },
  { name: 'Custom Brand — Rozwiązania na zamówienie', moq: 'indywidualnie według zakresu projektu', strengths: 'pełna personalizacja • identyfikacja marki • opakowanie • zestawy akcesoriów • konfiguracja oferty' },
];
      ...
      <main className="premium-home">
        ...
        <section className="commerce-hero">
          <div className="commerce-hero__brand">
            ...
            <p style={{ maxWidth: 560, lineHeight: 1.7, color: '#c9c9c9' }}>Profesjonalna obsługa przedsiębiorstw: dobór produktów, organizacja dostaw, zapytania ofertowe, rozwiązania private label oraz indywidualne wsparcie handlowe.</p>
          ...
          <aside className="commerce-hero__offer">
            ...
            <div style={{ marginTop: 22, padding: 18, border: '1px solid #38434a', borderRadius: 12, background: 'rgba(8,15,19,.72)' }}><strong style={{ color: '#f0d778' }}>WYCENA INDYWIDUALNA B2B</strong><p style={{ marginBottom: 0, color: '#b7b7b7', lineHeight: 1.55 }}>Cena, MOQ, konfiguracja, transport i warunki realizacji są potwierdzane dla konkretnego produktu i zamówienia.</p></div>
          </aside>
        </section>
        ...
        <section className="section featured-offers-section"><p className="eyebrow">NO NAME • NO LOGO • PRIVATE LABEL</p><h2>Serie produktów elektronicznych z miejscem na logo klienta</h2><p>Program OEM/ODM może obejmować logo na urządzeniu, ekran startowy, opakowanie, etykietę, a zależnie od projektu również firmware, pamięć, sieć i aplikacje.</p><div className="featured-offer-grid">{privateLabelPrograms.map((program, index) => (<article className="featured-offer-card" key={program.name}><img src={homeAssets0.noNameLogo} alt={`Private Label — miejsce na logo klienta — ${program.name}`} /><div className="featured-offer-copy"><span>OEM / ODM {index + 1}/{privateLabelPrograms.length}</span><h3>{program.name}</h3><p><strong>Możliwości:</strong> {program.strengths}.</p><p><strong>MOQ:</strong> {program.moq}.</p><p><strong>Branding klienta:</strong> zakres personalizacji jest potwierdzany dla wybranego modelu i wielkości zamówienia.</p><strong className="featured-price">Wycena indywidualna B2B</strong><Link href={`/offers/new?product=${encodeURIComponent(program.name)}`}>ZAPYTAJ O PRIVATE LABEL →</Link></div></article>))}</div></section>
        ...
        <section className="section premium-contact-block"><div><p className="eyebrow">Profesja Premium Limited</p><h2>Zapytaj o rozwiązanie dopasowane do Twojej firmy</h2><p>Produkty, sourcing, private label, import i finansowanie w jednym procesie obsługi B2B.</p></div><div className="cta-row"><Link href="/catalog"><button>Przejdź do katalogu</button></Link><Link href="/offers/new"><button className="cta-secondary">Złóż zapytanie B2B</button></Link></div></section>
      </main>
      <Footer />
    </>
  );
