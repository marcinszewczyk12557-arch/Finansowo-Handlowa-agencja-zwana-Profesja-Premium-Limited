...
export const franchiseCatalog: FranchiseProduct[] = catalogBlueprint.flatMap(([category, products], categoryIndex) =>
  products.map((title, productIndex) => ({
    ...
    category,
    title,
    use: `Indywidualnie konfigurowana oferta dla firm w kategorii ${category.toLowerCase()}; parametry, cena, dostępność i warunki dostawy są potwierdzane przed złożeniem wiążącej oferty.`,
